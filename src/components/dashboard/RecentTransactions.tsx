    ...
    <div className="space-y-6">
      {transactions.length === 0 ? (
        ...
        transactions.map((tx, index) => (
          ...
          >
            <Avatar className="h-10 w-10 border border-white/5 group-hover:border-primary/50 transition-colors">
              <AvatarFallback className="bg-primary/10 text-primary text-xs font-bold">
                {tx.category.name.substring(0, 2).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div className="ml-4 space-y-0.5">
              <p className="text-sm font-semibold leading-none group-hover:text-primary transition-colors">{tx.title}</p>
